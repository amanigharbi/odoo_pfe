...
class TimeTable(models.Model):
    ...
    timetable_mon = fields.One2many('time.table.line', 'table_id',
                                    domain=[('week_day', '=', 'Monday')])
    timetable_tue = fields.One2many('time.table.line', 'table_id',
                                    domain=[('week_day', '=', 'Tuesday')])
    timetable_wed = fields.One2many('time.table.line', 'table_id',
                                    domain=[('week_day', '=', 'Wednesday')])
    timetable_thur = fields.One2many('time.table.line', 'table_id',
                                     domain=[('week_day', '=', 'Thursday')])
    timetable_fri = fields.One2many('time.table.line', 'table_id',
                                    domain=[('week_day', '=', 'Friday')])
    timetable_sat = fields.One2many('time.table.line', 'table_id',
                                    domain=[('week_day', '=', 'Saturday')])
...
class TimeTableLine(models.Model):
    ...
    period_id=fields.Selection([('1','Period 1'),('2','Period 2'),('3','Period 3'),('4','Period 4'),('5','Period 5'),('6','Period 6'),],'Period')
        ...
        if timetable_rec:
            for data in timetable_rec:
                for record in data.timetable_ids:
                    ...
                    if (self.class_room_id == record.class_room_id ):
                            raise ValidationError(_("The room is occupied."))
                    #if(self.start_time == record.start_time):
                     #       raise ValidationError(_("There is session in this time."))
