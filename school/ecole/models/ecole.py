...
class AcademicYear(models.Model):
    ''' Defines an annee academique '''
    _name = "academic.year"
    _description = "annee academique"
    _order = "sequence"

    sequence = fields.Integer('Sequence', required=True,
                              help="Sequence order you want to see this year.")
    name = fields.Char('Name', required=True, help='Name of annee academique')
    code = fields.Char('Code', required=True, help='Code of annee academique')
    date_start = fields.Date('Start Date', required=True,
                             help='Starting date of annee academique')
    date_stop = fields.Date('End Date', required=True,
                            help='Ending of annee academique')
    month_ids = fields.One2many('academic.month', 'year_id', 'Months',
                                help="related Academic months")
    grade_id = fields.Many2one('grade.master', "Mention")
    ...
    def _check_academic_year(self):
        ...
        if delta.days > 365 and not calendar.isleap(new_start_date.year):
            raise ValidationError(_('''Error! The duration of the annee academique
                                      is invalid.'''))
        if (self.date_stop and self.date_start and
                self.date_stop < self.date_start):
            raise ValidationError(_('''The start date of the annee academique'
...
class AcademicMonth(models.Model):
    ''' Defining a month of an annee academique '''
    ...
    year_id = fields.Many2one('academic.year', 'annee academique', required=True,
                              help="Related annee academique ")
    description = fields.Text('Description')
...
class EcoleStandard(models.Model):
    ''' Defining a standard related to ecole '''
    _name = 'ecole.standard'
    _description = 'Ecole Standards'
    _rec_name = "standard_id"

    @api.depends('standard_id', 'ecole_id', 'division_id', 'medium_id',
                 'ecole_id')
    def _compute_eleve(self):
        '''Compute eleve of done state'''
    ...
    @api.depends("capacity", "total_eleves")
    def _compute_remain_seats(self):
        for rec in self:
            rec.remaining_seats = rec.capacity - rec.total_eleves
    ...
    eleve_ids = fields.One2many('eleve.eleve', 'standard_id',
                                  'eleve In Class',
    ...
    def check_standard_unique(self):
        standard_search = self.env['ecole.standard'
                                   ].search([('standard_id', '=',
                                              self.standard_id.id),
                                             ('division_id', '=',
                                              self.division_id.id),
    ...
    def unlink(self):
        for rec in self:
            if rec.eleve_ids or rec.subject_ids or rec.syllabus_ids:
                raise ValidationError(_('''You cannot delete this standard
                because it has reference with eleve or subject or
                syllabus!'''))
        return super(EcoleStandard, self).unlink()
...
class EcoleEcole(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company',
                                 ...
                                 required=True,
                                 delegate=True)
    com_name = fields.Char('Ecole Name', related='company_id.name',
                           store=True)
    code = fields.Char('Code', required=True)
    standards = fields.One2many('ecole.standard', 'ecole_id',
                                'Standards')
...
class SubjectSubject(models.Model):
    ...
    enseignant_ids = fields.Many2many('ecole.enseignant', 'subject_enseignant_rel',
                                   'subject_id', 'enseignant_id', 'enseignants')
    ...
    eleve_ids = fields.Many2many('eleve.eleve',
                                   'elective_subject_eleve_rel',
                                   'subject_id', 'eleve_id', 'eleves')
...
class SubjectSyllabus(models.Model):
    ...
    standard_id = fields.Many2one('ecole.standard', 'Standard')
...
class eleveHistory(models.Model):
    ...
    eleve_id = fields.Many2one('eleve.eleve', 'eleve')
    academice_year_id = fields.Many2one('academic.year', 'annee academique',
                                        )
    standard_id = fields.Many2one('ecole.standard', 'Standard')
...
class eleveCertificate(models.Model):
    _name = "eleve.certificate"
    _description = "eleve Certificate"
...
class elevePreviousEcole(models.Model):
    ''' Defining a eleve previous ecole information '''
    _name = "eleve.previous.ecole"
    _description = "eleve Previous Ecole"

    previous_ecole_id = fields.Many2one('eleve.eleve', 'eleve')
    name = fields.Char('Name', required=True)
...
class AcademicSubject(models.Model):
    ...
    add_sub_id = fields.Many2one('eleve.previous.ecole', 'Add Subjects',
                                 invisible=True)
    name = fields.Char('Name', required=True)
    maximum_marks = fields.Integer("Maximum marks")
    minimum_marks = fields.Integer("Minimum marks")
...
class eleveFamilyContact(models.Model):
    ''' Defining a eleve emergency contact information '''
    _name = "eleve.family.contact"
    _description = "eleve Family Contact"
    ...
    family_contact_id = fields.Many2one('eleve.eleve', 'eleve Ref.')
    rel_name = fields.Selection([('exist', 'Link to Existing eleve'),
                                 ('new', 'Create New Relative Name')],
                                'Related eleve', help="Select Name",
                                required=True)
    user_id = fields.Many2one('res.users', 'User ID', ondelete="cascade")
    stu_name = fields.Many2one('eleve.eleve', 'Existing eleve',
                               help="Select eleve From Existing List")
...
class eleveRelationMaster(models.Model):
    ''' eleve Relation Information '''
    _name = "eleve.relation.master"
    _description = "eleve Relation Master"
...
class GradeMaster(models.Model):
    ...
    name = fields.Char('Mention', required=True)
    grade_ids = fields.One2many('grade.line', 'grade_id', 'Mention Lines')
...
class GradeLine(models.Model):
    ...
    grade = fields.Char('Mention', required=True, help="Grade")
    ...
    grade_id = fields.Many2one("grade.master", 'Mention Ref.')
    name = fields.Char('Name')
...
class eleveReminder(models.Model):
    _name = 'eleve.reminder'
    _description = "eleve Reminder"
    ...
    stu_id = fields.Many2one('eleve.eleve', 'eleve Name', required=True,
                             default=check_user)
...
class eleveCast(models.Model):
    _name = "eleve.cast"
    _description = "eleve Cast"
...
class Report(models.Model):
    ...
    def render_template(self, template, values=None):
        ...
        if eleve_id and eleve_id.state == 'draft':
            raise ValidationError(_('''You cannot print report for
                eleve in unconfirm state!'''))
