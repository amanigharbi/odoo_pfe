...
class TimeTableLine(models.Model):
    ...
    teacher_id = fields.Many2one('ecole.teacher', 'Nom Ecole',
                                 help="Select Teacher")
    subject_id = fields.Many2one('subject.subject', 'Nom Matière',
                                 help="Select Subject")
    table_id = fields.Many2one('time.table', 'Calendrier')
...
class SubjectSubject2(models.Model):
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False,
        ...
        if teacher_id:
            for teacher_data in self.env['ecole.teacher'].browse(teacher_id):
